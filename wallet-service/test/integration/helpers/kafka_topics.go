...
func CreateTopics(ctx context.Context, t TB, broker string, topics ...string) {
	...
	// Redpanda exposes the controller via the Kafka protocol.
	...
	if err != nil {
		// Fall back to the original broker address if controller lookup fails.
		var dialErr error
	...
	if err := ctrlConn.CreateTopics(configs...); err != nil {
		// Ignore "topic already exists" errors — idempotent call.
...
// isTopicAlreadyExistsErr returns true for the Kafka error code 36 (TOPIC_ALREADY_EXISTS).
